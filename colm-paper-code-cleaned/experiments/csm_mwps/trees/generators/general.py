...
class GeneralMisconGenerator(Generator):
    def __init__(self, start_types: List[Type], inference_rules: List[InferenceRule], rule_sampling_policy: RuleSamplingPolicy, stopping_criterion: Criterion, 
                 max_part_whole: int = 4, comp_same_entity_prob: float = 0.5, compeq_same_entity_prob: float = 1.0, 
    ...
    def generate(self, seed: int = 14) -> ProofTree:
        ...
        for lf, valid_rules in self.expand_bfs(tree, root, property_tracker):
            rule = self.rule_sampling_policy.sample(lf, tree, valid_rules)
            parametrization = {}
            if isinstance(rule, ContTransferCont) or isinstance(rule, ContTransferContArithError):
            ...
            elif isinstance(rule, ContCompCont) or isinstance(rule, ContCompContArithError):
                assert isinstance(lf, Container), "Conclusion is expected to be Container"
            ...
            elif isinstance(rule, ContPartWhole) or isinstance(rule, ContPartWholeArithError):
    ...
    def create_start_lf(self, typ: Type, property_tracker: PropertyTracker, use_attribute: bool, use_unit: bool) -> LogicalForm:
        if typ == Container:
            ...
            n = random.randint(2, self.max_part_whole) # NOTE: the current templates only support >1 part
            part_agents = [
                property_tracker.request_id(PropertyType.AGENT)
                for _ in range(n)
            ]
