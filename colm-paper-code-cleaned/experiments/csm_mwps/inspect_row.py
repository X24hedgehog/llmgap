...
def print_model_results(row, task):
    model_cols = [c for c in row.index if c not in {
        ...
        "target_answer", "split",
        "correct_answer", "distractor_answers", "distractor_reasoning_traces",
        "misconception_type", "target_distractor_answers",
...
def main():
    ...
    if args.task == "distractor":
        ...
        hr()
        # distractor_answers is a JSON list
        raw_answers = row.get("distractor_answers", "[]")
        try:
            answers = json.loads(raw_answers)
        except (ValueError, TypeError):
            answers = [raw_answers]
        print(f"  DISTRACTOR ANSWERS ({len(answers)}):")
        for i, ans in enumerate(answers):
            print(f"    [{i}] {ans}")
        hr()
        raw_rts = row.get("distractor_reasoning_traces", "[]")
        try:
            rts = json.loads(raw_rts)
        except (ValueError, TypeError):
            rts = [raw_rts]
        for i, rt in enumerate(rts):
            print(f"  DISTRACTOR REASONING TRACE [{i}]:")
            for line in str(rt).splitlines():
                print("   ", line)
            if i < len(rts) - 1:
                print()
