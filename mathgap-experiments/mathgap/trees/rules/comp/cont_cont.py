...
class ContContComp(InferenceRule):
    ...
    def twisted_infer_knowledge(self, premises: List[LogicalForm], conclusion: LogicalForm):
        ...
        # twisted: flip the direction (use obj - subj rather than subj - obj)
        if conclusion.comp_type == ComparisonType.MORE_THAN:
            conclusion.quantity = Subtraction(obj_container.quantity, subj_container.quantity)
        elif conclusion.comp_type == ComparisonType.LESS_THAN:
            conclusion.quantity = Subtraction(subj_container.quantity, obj_container.quantity)
        ...
        assert conclusion.quantity is not None, f"Conclusion.quantity must be set after twisted inferring knowledge! conclusion.comp_type={conclusion.comp_type}"