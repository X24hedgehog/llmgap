...
from mathgap.expressions import Sum, Subtraction
...
class ContPartWhole(InferenceRule):
    ...
    # def twisted_infer_knowledge(self, premises: List[Container], conclusion: LogicalForm):
    #     assert all([isinstance(c, Container) for c in premises]), "All premises are expected to be a Container"
    #     assert isinstance(conclusion, PartWhole), "Conclusion is expected to be a PartWhole"
    #     assert all(c.quantity is not None for c in premises), "No container quantity can be None"

    #     if len(premises) == 2:
    #         q1 = premises[0].quantity
    #         q2 = premises[1].quantity
    #         # twisted: instead of sum, do abs difference (max - min)
    #         if q1 >= q2:
    #             conclusion.quantity = Subtraction(q1, q2)
    #         else:
    #             conclusion.quantity = Subtraction(q2, q1)
    #     else:
    #         raise NotImplementedError("twisted infer_knowledge for PartWhole is implemented only for 2 agents")

    #     assert conclusion.quantity is not None, f"Conclusion.quantity must be set after twisted inferring knowledge!"