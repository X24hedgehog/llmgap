...
class ContTransferCont(InferenceRule):
    ...
    def twisted_infer_knowledge(self, premises: List[LogicalForm], conclusion: LogicalForm):
        ...
        if conclusion.agent == transfer.receiver:
            # twisted: receiver loses instead of gains
            conclusion.quantity = Subtraction(container.quantity, transfer.quantity)
        elif conclusion.agent == transfer.sender:
            # twisted: sender gains instead of loses
            conclusion.quantity = Addition(container.quantity, transfer.quantity)

        assert conclusion.quantity is not None, f"Conclusion.quantity must be set after twisted inferring knowledge! conclusion.agent={conclusion.agent}, transfer.receiver={transfer.receiver}, transfer.sender={transfer.sender}"
