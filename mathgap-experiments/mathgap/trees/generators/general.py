...
class GeneralGenerator(Generator):
    ...
    def generate_twist_one_node(self, seed: int = 14):
        """
        Generate one tree and produce all single-node-twisted variants.
        Returns:
            base_tree: ProofTree (correct)
            twisted_trees: dict[node_id -> ProofTree] where each tree has exactly that node
                           computed with twisted_infer_knowledge, all others normal.
        """
        base_tree = self.generate(seed=seed)
        twisted_trees = base_tree.compute_symbolically_twist_one_node()
