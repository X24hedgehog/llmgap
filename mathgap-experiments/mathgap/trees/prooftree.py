...
class ProofTree:
    ...
    def compute_symbolically_twist_one_node(self):
        """
        Returns a dict mapping twisted node id -> ProofTree copy with exactly that node computed via
        `twisted_infer_knowledge`, while all other nodes use normal `infer_knowledge`.

        Eligible twisted nodes:
        - non-leaf
        - node.rule has twisted_infer_knowledge
        """
        results = {}
        for node in self.traverse(TraversalOrder.TIME):
            if node.is_leaf:
            ...
            if not hasattr(node.rule, "twisted_infer_knowledge"):
                continue


            target_id = self.id_by_node[node]
            tree_copy = self.copy()

            for n in tree_copy.traverse(TraversalOrder.TIME):
                if n.is_leaf:
                    continue
                if n.rule is None:
                    continue

                if tree_copy.id_by_node[n] == target_id:
                    # twisted for this node only
                    twisted_fn = getattr(n.rule, "twisted_infer_knowledge", None)
                    if twisted_fn is None:
                        n.rule.infer_knowledge(n.premises, n.logicalform)
                    else:
                        twisted_fn(n.premises, n.logicalform)
                else:
                    n.rule.infer_knowledge(n.premises, n.logicalform)

            tree_copy.is_symbolically_computed = True
            results[target_id] = tree_copy
